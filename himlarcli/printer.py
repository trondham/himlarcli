...
class Printer(object):
    ...
    def prettyprint_project_metadata(project, options, logger, regions, user=None):
        ...
        # Disabled project?
        status = Color.fg.red + "**DISABLED** " + Color.reset if not (project.enabled or project.enabled == 'True') else ''
        ...
        if user is not None and not options.admin:
            prole = Color.fg.red + 'admin' + Color.reset if user == project_admin else Color.fg.GRN + 'member' + Color.reset
            out_str += f" ({prole})"
